...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "p256") {
    const p256 = envelope;
    return concat2(serializedP256Type, fromNumber(p256.signature.r, { size: 32 }), fromNumber(p256.signature.s, { size: 32 }), fromNumber(p256.publicKey.x, { size: 32 }), fromNumber(p256.publicKey.y, { size: 32 }), fromNumber(p256.prehash ? 1 : 0, { size: 1 }), options.magic ? magicBytes : "0x");
  }
  if (type2 === "webAuthn") {
    const webauthn = envelope;
